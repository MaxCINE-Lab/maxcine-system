...
function run(command, args, cwd = root, env = process.env) { const result = spawnSync(command, args, { cwd, env, stdio: 'inherit' }); if (result.status !== 0) process.exit(result.status ?? 1); }
...
const api = spawn('npx', ['wrangler', 'dev', '--config', 'wrangler.toml', '--local', '--port', '8791', '--persist-to', persistence, '--var', 'APP_ORIGIN:http://127.0.0.1:5175'], { cwd: join(root, 'apps/api'), stdio: 'inherit' });
const web = spawn('npx', ['vite', '--host', '127.0.0.1', '--port', '5175', '--strictPort'], { cwd: join(root, 'apps/web'), stdio: 'inherit', env: { ...process.env, VITE_API_BASE_URL: 'http://127.0.0.1:8791' } });
const stop = () => { for (const child of [api, web]) if (!child.killed) child.kill('SIGTERM'); };
try { await waitFor('http://127.0.0.1:8791/health'); await waitFor('http://127.0.0.1:5175'); run('npx', ['playwright', 'test', '--config=e2e/playwright.config.mjs']); } finally { stop(); }
