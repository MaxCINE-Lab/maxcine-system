...
function parseCustomerRiskSmartInput(value: string): Partial<CustomerRiskDraft> {
  ...
  const telegram = pick([/(?:Telegram|TG|电报)[:：\s]*(@?[A-Za-z0-9_]{3,40})/i]);
  const qqNickname = pick([/(?:QQ|qq)[:：\s]*([1-9]\d{4,11})/]);
  const whatsapp = pick([/(?:WhatsApp|WA)[:：\s]*(\+?[0-9][0-9\s-]{5,24})/i]).replace(/[^\d+]/g, '');
  ...
  const platformNickname = labelledNickname || lines.find((line) => /^(?:tbNick_)?[A-Za-z0-9][A-Za-z0-9_.-]{2,79}$/.test(line) && line !== phone && line !== qqNickname && line !== telegram && line !== whatsapp && line !== ipv4) || '';
  const keyword = [city, addressLine.match(/科技园|小区|大厦|黑名单|同行|骗保|砍价|退款|辱骂/)?.[0] ?? ''].filter(Boolean).join(' ');
  return { name: nameLine, recipientName: nameLine, phone, platformNickname, wechatNickname, qqNickname, telegram, whatsapp, shippingAddress: addressLine, city, ipLocation, keyword };
}
...
function CustomerRiskCenter({ user, route }: { user: SessionUser; route: string }) {
  ...
  const runSearch = async (value = searchInput) => {
    ...
    try {
      ...
      setItems(matches);
      if (matches.length === 1 && (matches[0].priority ?? 9) <= 4) await loadDetail(matches[0].id);
  ...
  const saveBlacklist = async () => {
    ...
    if (!blacklistReasons.length) return setNotice({ tone: 'error', text: '请选择至少一个风险原因。' });
  ...
  return <DealerShell user={user} route={route} title="Customer Risk Center" subtitle="当前仅适用于 MaxCINE Mavic 4 Pro 增广镜。"><Alert notice={notice} /><section className="panel risk-home"><div className="risk-mode-switch" role="tablist" aria-label="客户风控操作"><button className={mode === 'search' ? 'is-active' : ''} onClick={() => { setMode('search'); setNotice(null); }}>模糊查询</button><button className={mode === 'create' ? 'is-active' : ''} onClick={() => { setMode('create'); setNotice(null); }}>新建黑名单</button></div>{mode === 'search' ? <div className="risk-spotlight"><h2>快速查询客户风险</h2><p>输入平台昵称、手机号、姓名、地址、IP 所属地等任意信息。</p><form onSubmit={(event) => { event.preventDefault(); void runSearch(); }}><input autoFocus value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="输入平台昵称、手机号、姓名、地址、IP 所属地等任意信息" /><ActionButton type="submit" disabled={loading}>{loading ? '查询中…' : '查询'}</ActionButton></form>{tbNickSuggestion && <button className="risk-suggestion" onClick={() => { setSearchInput(tbNickSuggestion); void runSearch(tbNickSuggestion); }}>使用 {tbNickSuggestion} 查询</button>}</div> : <div className="risk-spotlight"><h2>新建共享黑名单</h2><p>粘贴客户昵称、手机号、姓名、地址、IP 省份等内容，系统将自动识别。</p><textarea autoFocus value={createRaw} onChange={(event) => setCreateRaw(event.target.value)} placeholder={'tbNick_91xpa\n何满堂\n18191316611\n陕西省渭南市大荔县城关街道东大街21号\nIP：陕西省'} /><ActionButton disabled={loading} onClick={() => void recognizeBlacklist()}>自动识别</ActionButton></div>}</section>{mode === 'search' && hasSearched && !detail && items.length > 1 && <section className="panel risk-results"><div className="panel-title"><h2>可能匹配的客户</h2><span>{items.length} 条</span></div><div className="risk-result-list">{items.map((item) => <button key={item.id} onClick={() => void loadDetail(item.id)}><strong>{displayTitle(item)}</strong><span>{item.displayName || item.recipientName || '未填写姓名'} · {phoneTail(item.phone) || '无手机号'}</span><small>{addressSummary(item.shippingAddress || item.city || '') || '暂无地址'} {item.ipLocation ? `· IP ${item.ipLocation}` : ''}</small><em>{item.statusText} · 最近 {dateTime(item.lastConsultedAt)}</em></button>)}</div></section>}{mode === 'search' && hasSearched && !detail && !items.length && <Empty title="未找到匹配的客户风险档案。" text="如确认该客户存在风险，可切换到“新建黑名单”登记。" />}{detail && <section className="risk-profile"><article className="panel risk-profile-card"><div className="risk-profile-head"><div><h2>{displayTitle(detail.customer)}</h2><p>{detail.customer.statusText} · 风险等级：{detail.customer.riskLevelText}</p></div>{isManager && <ActionButton secondary onClick={startProfileEdit}>编辑档案</ActionButton>}</div><div className="risk-info-grid">{infoRows.map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div></article>{profileEdit && isManager && <section className="panel risk-edit-panel"><div className="panel-title"><h2>编辑客户档案</h2><button className="table-action" onClick={() => setProfileEdit(null)}>取消</button></div><div className="form-layout"><label>平台昵称<input value={profileEdit.platformNickname} onChange={(event) => setProfileEdit({ ...profileEdit, platformNickname: event.target.value })} /></label><label>手机号<input value={profileEdit.phone} onChange={(event) => setProfileEdit({ ...profileEdit, phone: event.target.value })} /></label><label>姓名<input value={profileEdit.name} onChange={(event) => setProfileEdit({ ...profileEdit, name: event.target.value })} /></label><label>收件人<input value={profileEdit.recipientName} onChange={(event) => setProfileEdit({ ...profileEdit, recipientName: event.target.value })} /></label><label>IP 信息<input value={profileEdit.ipLocation} onChange={(event) => setProfileEdit({ ...profileEdit, ipLocation: event.target.value })} /></label><label>状态<select value={profileEdit.status} onChange={(event) => setProfileEdit({ ...profileEdit, status: event.target.value as CustomerRiskStatus })}>{riskStatusOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>风险等级<select value={profileEdit.riskLevel} onChange={(event) => setProfileEdit({ ...profileEdit, riskLevel: event.target.value as CustomerRiskLevel })}>{riskLevelOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label></div><label>地址<textarea value={profileEdit.shippingAddress} onChange={(event) => setProfileEdit({ ...profileEdit, shippingAddress: event.target.value })} /></label><label>管理员备注<textarea value={profileEdit.customerNote} onChange={(event) => setProfileEdit({ ...profileEdit, customerNote: event.target.value })} /></label><div><span className="form-label">风险原因</span><div className="checkbox-grid">{riskReasonOptions.map((reason) => <label key={reason}><input type="checkbox" checked={profileEdit.riskReasons.includes(reason)} onChange={(event) => setProfileEdit({ ...profileEdit, riskReasons: event.target.checked ? Array.from(new Set([...profileEdit.riskReasons, reason])) : profileEdit.riskReasons.filter((item) => item !== reason) })} />{reason}</label>)}</div></div><div className="action-list"><ActionButton disabled={saving} onClick={() => void saveProfileEdit()}>{saving ? '保存中…' : '保存档案'}</ActionButton></div></section>}<section className="panel risk-section"><h3>风险原因</h3><div className="risk-tags">{detail.customer.riskReasons.length ? detail.customer.riskReasons.map((reason) => <span key={reason}>{reason}</span>) : <small>暂无风险原因</small>}</div></section>{detail.customer.note && <section className="panel risk-section"><h3>管理员备注</h3><p>{detail.customer.note}</p></section>}<section className="panel risk-section"><div className="panel-title"><h2>咨询历史</h2><span>{detail.events.length} 条</span></div><ul className="risk-timeline">{detail.events.map((event) => <li key={event.id}><time>{dateTime(event.happenedAt)}</time><div><strong>{event.dealerName || event.storeName || '官方'} · {event.createdByName || '—'} · {riskResultLabel(event.consultationResult)}</strong><p>{event.riskReasons.join('、') || '无风险原因'}{event.otherReason ? `；${event.otherReason}` : ''}</p>{event.note && <small>{event.note}</small>}{isManager && <div className="inline-actions"><button className="table-action" onClick={() => setEditingEvent({ ...event, happenedDate: event.happenedAt.slice(0, 10) })}>编辑</button><button className="table-action table-action--danger" onClick={() => void deleteEvent(event.id)}>删除</button></div>}</div></li>)}</ul></section>{editingEvent && isManager && <section className="panel risk-edit-panel"><div className="panel-title"><h2>编辑咨询记录</h2><button className="table-action" onClick={() => setEditingEvent(null)}>取消</button></div><div className="form-layout"><label>咨询结果<select value={editingEvent.consultationResult} onChange={(event) => setEditingEvent({ ...editingEvent, consultationResult: event.target.value as CustomerRiskResult })}>{riskResultOptions.map((value) => <option key={value} value={value}>{riskResultLabel(value)}</option>)}</select></label><label>咨询日期<input type="date" value={editingEvent.happenedDate} onChange={(event) => setEditingEvent({ ...editingEvent, happenedDate: event.target.value })} /></label></div><div><span className="form-label">风险原因</span><div className="checkbox-grid">{riskReasonOptions.map((reason) => <label key={reason}><input type="checkbox" checked={editingEvent.riskReasons.includes(reason)} onChange={(event) => toggleEditReason(reason, event.target.checked)} />{reason}</label>)}</div></div><label>咨询备注<textarea value={editingEvent.note} onChange={(event) => setEditingEvent({ ...editingEvent, note: event.target.value })} /></label><ActionButton disabled={saving} onClick={() => void saveEventEdit()}>{saving ? '保存中…' : '保存记录'}</ActionButton></section>}<section className="panel risk-section"><div className="panel-title"><h2>新增咨询记录</h2>{!newEventOpen && <button className="table-action" onClick={() => setNewEventOpen(true)}>新增</button>}</div>{newEventOpen ? <div className="risk-compact-form"><label>咨询结果<select value={eventDraft.consultationResult} onChange={(event) => setEventDraft({ ...eventDraft, consultationResult: event.target.value as CustomerRiskResult })}>{riskResultOptions.map((value) => <option key={value} value={value}>{riskResultLabel(value)}</option>)}</select></label><div><span className="form-label">风险原因</span><div className="checkbox-grid">{riskReasonOptions.map((reason) => <label key={reason}><input type="checkbox" checked={eventDraft.riskReasons.includes(reason)} onChange={(event) => toggleDraftReason(reason, event.target.checked)} />{reason}</label>)}</div></div><label>本次咨询备注<textarea value={eventDraft.note} onChange={(event) => setEventDraft({ ...eventDraft, note: event.target.value })} placeholder="记录本次咨询的关键情况" /></label><div className="action-list"><ActionButton disabled={saving} onClick={() => void saveConsultation()}>{saving ? '保存中…' : '保存咨询记录'}</ActionButton><ActionButton secondary onClick={() => setNewEventOpen(false)}>取消</ActionButton></div></div> : <p className="soft-text">只记录本次咨询结果、风险原因和备注，不重复填写客户身份信息。</p>}</section></section>}{mode === 'create' && recognized && <section className="panel risk-create-confirm"><div className="panel-title"><h2>确认识别结果</h2><span>状态固定为共享黑名单</span></div><div className="risk-info-grid">{recognized.platformNickname && <label><span>平台昵称</span><div className="risk-prefix-row"><select value={nicknamePrefix} onChange={(event) => setNicknamePrefix(event.target.value)}><option value="tbNick_">tbNick_</option><option value="">无前缀</option></select><input value={recognized.platformNickname.toLowerCase().startsWith('tbnick_') ? recognized.platformNickname : recognized.platformNickname.replace(/^tbNick_/i, '')} onChange={(event) => setRecognizedValue('platformNickname', platformNicknameWithPrefix(nicknamePrefix, event.target.value))} /></div></label>}{recognized.phone && <label><span>手机号</span><input value={recognized.phone} onChange={(event) => setRecognizedValue('phone', event.target.value)} /></label>}{recognized.name && <label><span>姓名</span><input value={recognized.name} onChange={(event) => { setRecognizedValue('name', event.target.value); setRecognizedValue('recipientName', event.target.value); }} /></label>}{recognized.shippingAddress && <label><span>地址</span><textarea value={recognized.shippingAddress} onChange={(event) => setRecognizedValue('shippingAddress', event.target.value)} /></label>}{recognized.ipLocation && <label><span>IP 信息</span><input value={recognized.ipLocation} onChange={(event) => setRecognizedValue('ipLocation', event.target.value)} /></label>}</div><button className="table-action" onClick={() => setShowSupplement(!showSupplement)}>{showSupplement ? '收起补充信息' : '补充信息'}</button>{showSupplement && <div className="form-layout"><label>微信昵称<input value={recognized.wechatNickname ?? ''} onChange={(event) => setRecognizedValue('wechatNickname', event.target.value)} /></label><label>QQ<input value={recognized.qqNickname ?? ''} onChange={(event) => setRecognizedValue('qqNickname', event.target.value)} /></label><label>Telegram<input value={recognized.telegram ?? ''} onChange={(event) => setRecognizedValue('telegram', event.target.value)} /></label><label>城市<input value={recognized.city ?? ''} onChange={(event) => setRecognizedValue('city', event.target.value)} /></label></div>}{duplicateMatches.length > 0 && <div className="risk-duplicate-box"><strong>发现现有客户档案</strong><p>为避免重复建档，请先查看现有档案，或直接为现有客户新增咨询记录。</p>{duplicateMatches.map((item) => <div key={item.id}><span>{displayTitle(item)} · {item.statusText}</span><div className="inline-actions"><button className="table-action" onClick={() => { setMode('search'); void loadDetail(item.id); }}>查看现有档案</button><button className="table-action" onClick={() => { setMode('search'); void loadDetail(item.id, true); }}>为现有客户新增咨询记录</button></div></div>)}</div>}<div className="form-layout"><label>风险等级<select value={blacklistRiskLevel} onChange={(event) => setBlacklistRiskLevel(event.target.value as CustomerRiskLevel)}>{riskLevelOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label></div><div><span className="form-label">风险原因</span><div className="checkbox-grid">{riskReasonOptions.map((reason) => <label key={reason}><input type="checkbox" checked={blacklistReasons.includes(reason)} onChange={(event) => toggleBlacklistReason(reason, event.target.checked)} />{reason}</label>)}</div></div><label>管理员备注或档案备注<textarea value={blacklistNote} onChange={(event) => setBlacklistNote(event.target.value)} /></label><label>本次咨询备注<textarea value={blacklistEventNote} onChange={(event) => setBlacklistEventNote(event.target.value)} /></label><div className="sticky-action"><span>保存后将创建共享黑名单档案，并记录本次咨询。</span><ActionButton disabled={saving || (duplicateMatches.length > 0 && !isManager)} onClick={() => void saveBlacklist()}>{saving ? '创建中…' : '创建黑名单档案'}</ActionButton></div></section>}</DealerShell>;
}
...
function AfterSalesSubmitForm({ user, route }: { user: SessionUser; route: string }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!description.trim() || !contactName.trim() || !contactPhone.trim() || !contactEmail.trim() || !contactAddress.trim()) return setNotice({ tone: 'error', text: '请完整填写问题描述和客户联系方式。' });
    setSaving(true); setNotice(null);
    try {
