  ...
  return <Shell user={user} route={route} logout={logout} title="售后管理" text="审核工单、分配服务中心、审核检测结果并生成报价。">
    ...
    {showMaterials ? <RepairMaterialsPanel notice={setNotice} /> : <>
      ...
      {selected && <section className="panel after-sales-detail">
        ...
        {['WAITING_PAYMENT_CONFIRMATION', 'WAITING_CUSTOMER_CONFIRMATION'].includes(selected.case.serviceStage) && <section><h3>确认收款</h3><p className="hint">该工单等待客户确认或付款。客户付款到账后，由管理员确认收款，工单将进入等待维修与发货流程。</p><div className="action-list"><Button onClick={() => void confirmPayment()}>确认已收款</Button></div></section>}
