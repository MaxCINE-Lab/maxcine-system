...
function Dealers({ user, route, logout }: Props) { const options = useOptions(); const [items, setItems] = useState<Dealer[] | null>(null); const [form, setForm] = useState(freshDealer()); const [editing, setEditing] = useState<string | null>(null); const [notice, setNotice] = useState<Notice>(null); const load = () => api<{ dealers: Dealer[] }>('/admin/dealers').then((data) => setItems(data.dealers)).catch((e) => setNotice({ tone: 'error', text: errorText(e) })); useEffect(() => { void load(); }, []); const submit = async (e: FormEvent) => { e.preventDefault(); if (!form.code || !form.name) return setNotice({ tone: 'error', text: '请填写经销商名称和编码。' }); try { await api(editing ? `/admin/dealers/${editing}` : '/admin/dealers', { method: editing ? 'PATCH' : 'POST', body: JSON.stringify({ ...form, notificationEmail: form.notificationEmail.trim(), serviceCenterId: form.serviceCenterId || null }) }); setNotice({ tone: 'success', text: '经销商资料已保存。' }); setEditing(null); setForm(freshDealer()); void load(); } catch (err) { setNotice({ tone: 'error', text: errorText(err) }); } }; const edit = async (id: string) => { const detail = await api<{ dealer: Dealer & { serviceCenterId: string | null } }>(`/admin/dealers/${id}`); setEditing(id); setForm({ code: detail.dealer.code, name: detail.dealer.name, province: detail.dealer.province, authorizationType: detail.dealer.authorizationType, serviceCenterId: detail.dealer.serviceCenterId || '', contactName: detail.dealer.contactName, notificationEmail: detail.dealer.notificationEmail || '', status: detail.dealer.status }); }; return <Shell user={user} route={route} logout={logout} title="经销商管理" text="维护授权资格、服务中心关联与业务范围。"><Feedback notice={notice} /><section className="toolbar"><a className="button button--secondary" href="#/system/admin/stores">店铺管理</a><a className="button button--secondary" href="#/system/admin/users">编辑用户授权</a></section><form className="panel" onSubmit={submit}><h2>{editing ? '编辑经销商' : '新增经销商'}</h2><div className="form-layout"><label>经销商名称<input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></label><label>经销商编码<input value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value.toUpperCase() })} /></label><label>省份<input value={form.province} onChange={(e) => setForm({ ...form, province: e.target.value })} /></label><label>授权类型<input value={form.authorizationType} onChange={(e) => setForm({ ...form, authorizationType: e.target.value })} /></label><label>关联授权服务中心<select value={form.serviceCenterId} onChange={(e) => setForm({ ...form, serviceCenterId: e.target.value })}><option value="">不关联</option>{options?.serviceCenters.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>负责人<input value={form.contactName} onChange={(e) => setForm({ ...form, contactName: e.target.value })} /></label><label>通知邮箱<input type="email" value={form.notificationEmail} onChange={(e) => setForm({ ...form, notificationEmail: e.target.value })} placeholder="订单发货后通知该邮箱，可为空" /></label><label>状态<select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}><option value="active">启用</option><option value="inactive">停用</option></select></label></div><Button type="submit">保存</Button></form>{!items ? <p>正在加载…</p> : <div className="table-wrap"><table><thead><tr><th>经销商</th><th>省份</th><th>授权类型</th><th>服务中心</th><th>通知邮箱</th><th>店铺／用户</th><th>状态</th><th>操作</th></tr></thead><tbody>{items.map((item) => <tr key={item.id}><td>{item.name}<br /><small>{item.code}</small></td><td>{item.province || '—'}</td><td>{item.authorizationType}</td><td>{item.serviceCenterName || '—'}</td><td>{item.notificationEmail || '—'}</td><td>{item.storeCount} / {item.userCount}</td><td>{item.status === 'active' ? '已启用' : '已停用'}</td><td><Button secondary onClick={() => void edit(item.id)}>查看／编辑</Button></td></tr>)}</tbody></table></div>}</Shell>; }
...
function Users({ user, route, logout }: Props) {
  ...
  const save = async () => {
    ...
    try {
      ...
      toast({ tone: 'success', text: '用户权限和授权范围已更新，旧登录状态已失效。' });
      setSelected(null);
      void load();
    } catch (e) { const text = errorText(e); setNotice({ tone: 'error', text }); toast({ tone: 'error', text }); }
  };
  const reset = async () => {
    if (!selected) return;
    ...
    try {
      ...
      toast({ tone: 'success', text: '密码已重置，用户下次登录必须修改密码。' });
    } catch (e) { const text = errorText(e); setNotice({ tone: 'error', text }); toast({ tone: 'error', text }); }
  };
  const updateWatermark = async (item: User, enabled: boolean) => {
    ...
    try {
      ...
      toast({ tone: 'success', text: `${item.name} 的页面水印已${enabled ? '开启' : '关闭'}。` });
    } catch (e) {
...
const serviceProgressSteps = [
  ...
  { key: 'repair', label: '等待维修与发货', stages: ['READY_FOR_PROCESSING', 'WAITING_REPAIR_SHIPMENT'] },
...
type AfterSalesPhase = 'overview' | (typeof serviceProgressSteps)[number]['key'];
function phaseForStage(stage: string): AfterSalesPhase {
  return serviceProgressSteps[serviceProgressIndex(stage)]?.key ?? 'overview';
}
...
function AfterSalesV2({ user, route, logout }: Props) {
  ...
  const [detailPhase, setDetailPhase] = useState<AfterSalesPhase>('overview');
  ...
  const filtered = items?.filter((item) => stage === 'all' || item.serviceStage === stage || (stage === 'PENDING_QUOTE' && item.serviceStage === 'PENDING_ADMIN_INSPECTION_REVIEW') || (stage === 'REPAIR_AND_SHIPMENT' && ['READY_FOR_PROCESSING', 'WAITING_REPAIR_SHIPMENT'].includes(item.serviceStage))) ?? [];
  ...
  const confirmPayment = async () => {
    ...
    try {
      ...
      setNotice({ tone: 'success', text: '已确认收款，工单已进入等待维修与发货流程。' });
      await open(selected.case.id);
      void load();
    } catch (error) {
      setNotice({ tone: 'error', text: errorText(error) });
  ...
  const confirmQuoteSend = async () => {
    ...
    try {
      ...
      const successText = quotePreview.quote.totalCents <= 0
        ? '报价已锁定并发送至客户邮箱。金额为 0 元，工单已自动进入等待维修与发货流程。'
  ...
  return <Shell user={user} route={route} logout={logout} title="售后管理" text="审核工单、分配服务中心、审核检测结果并生成报价。">
    ...
    {showMaterials ? <RepairMaterialsPanel notice={setNotice} /> : <>
      <div className="filter-row">{[['all', '全部'], ['PENDING_ADMIN_REVIEW', '待审核'], ['WAITING_CUSTOMER_SHIPMENT', '待寄修'], ['WAITING_SERVICE_CENTER_RECEIPT', '待收货'], ['INSPECTION_IN_PROGRESS', '检测中'], ['PENDING_QUOTE', '待出报价'], ['WAITING_PAYMENT_CONFIRMATION', '确认收款'], ['REPAIR_AND_SHIPMENT', '等待维修与发货'], ['RETURN_SHIPPED', '售后已发货']].map(([value, label]) => <button key={value} className={`filter ${stage === value ? 'active' : ''}`} onClick={() => setStage(value)}>{label}</button>)}</div>
      ...
      {selected && <section className="panel after-sales-detail" data-phase={detailPhase}>
        ...
        <nav className="after-sales-phase-nav" aria-label="售后阶段页面">
          <button type="button" className={detailPhase === 'overview' ? 'active' : ''} onClick={() => setDetailPhase('overview')}>总览</button>
          {serviceProgressSteps.map((step, index) => {
            const currentIndex = serviceProgressIndex(selected.case.serviceStage);
            const disabled = index > currentIndex;
            return <button type="button" key={step.key} className={detailPhase === step.key ? 'active' : ''} disabled={disabled} title={disabled ? '该工单尚未到达该阶段' : step.label} onClick={() => setDetailPhase(step.key as AfterSalesPhase)}>{step.label}</button>;
          })}
        </nav>
        ...
        {['WAITING_PAYMENT_CONFIRMATION', 'WAITING_CUSTOMER_CONFIRMATION'].includes(selected.case.serviceStage) && <section className="after-sales-phase after-sales-phase--quote"><h3>确认收款</h3><p className="hint">该工单等待客户确认或付款。客户付款到账后，由管理员确认收款，工单将进入等待维修与发货流程。</p><div className="action-list"><Button onClick={() => void confirmPayment()}>确认已收款</Button></div></section>}
        <section className="after-sales-phase after-sales-phase--repair after-sales-phase--ship_out"><h3>售后发货</h3>{['READY_FOR_PROCESSING', 'WAITING_REPAIR_SHIPMENT'].includes(selected.case.serviceStage) ? <><p className="hint">{selected.case.serviceStage === 'READY_FOR_PROCESSING' ? '该工单报价为 0 元，产品服务报告书发送后已自动进入等待维修与发货流程。处理完成后在这里记录发货。' : '管理员已确认收款，请继续安排维修、替换或后续发货。'}发货时间会按点击“确认售后发货”时的系统时间自动记录；照片仅用于系统内留档，不会发送给客户。</p><div className="form-layout"><label>快递公司<input value={outboundCarrier} onChange={(event) => setOutboundCarrier(event.target.value)} /></label><label>快递单号<input value={outboundTracking} onChange={(event) => setOutboundTracking(event.target.value)} placeholder="可为空" /></label><label>产品 SN（如有）<input value={outboundSerial} onChange={(event) => setOutboundSerial(event.target.value)} placeholder="可为空" /></label><label>发货邮件收件邮箱<input type="email" value={outboundRecipientEmail} onChange={(event) => setOutboundRecipientEmail(event.target.value)} placeholder="填写本次发货邮件收件邮箱" /></label></div><div className="service-photo-preview-grid admin-after-sales-photos">{outboundPhotoSlots.map((item) => {
