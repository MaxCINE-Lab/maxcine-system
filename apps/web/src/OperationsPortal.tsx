...
function OrderPage({ user, route, logout, warehouse = false, orderId }: Props & { warehouse?: boolean; orderId: string }) {
  ...
  const ship = async () => {
    ...
    const missingPhoto = shipmentPhotoRequirements.find((requirement) => !shipmentPhotos.some((photo) => photo.category === requirement.category));
    if (missingPhoto) return setNotice({ tone: 'error', text: `确认发货前请上传${missingPhoto.label}。` });
  ...
  return <Shell user={user} route={route} title={warehouse ? '发货订单' : '订单详情'} subtitle={data?.order.orderNo ?? '正在加载订单'} logout={logout}>
    ...
    {!data ? <p>正在加载…</p> : <div className="order-layout">
      ...
      {canFulfill && canShip && <section className="panel"><div className="panel-title"><h2>确认发货</h2><span>{warehouse ? '快递单号可为空，产品 SN 必填。' : '管理员也可以在审核通过后直接确认发货。'}</span></div><dl className="detail-grid"><dt>收货信息</dt><dd>{data.order.shippingAddress || '—'}</dd><dt>包装材料</dt><dd>{packageMaterials.join('、') || data.order.packageMaterials || '—'}</dd></dl><label>快递公司<input value={carrier} onChange={(event) => setCarrier(event.target.value)} /></label><label>快递单号<div className="scan-field"><input value={tracking} onChange={(event) => setTracking(event.target.value)} placeholder="可扫描或手动填写，非必填" /><Button secondary onClick={() => void scanTrackingNumber()}>扫描快递单号</Button></div></label>{warehouse ? <label>产品 SN<div className="scan-field scan-field--stacked"><textarea value={serialText} onChange={(event) => setSerialText(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') event.stopPropagation(); }} placeholder="一行一个 SN，扫描枪回车会自动换行" /><Button secondary onClick={() => void scanSerialNumber()}>扫描产品 SN</Button></div></label> : <dl className="detail-grid"><dt>本次发货 SN</dt><dd>{selectedSerials.join('、') || '请先在履约安排中选择或随机分配 SN。'}</dd></dl>}<div className="panel-title"><h2>出库照片</h2><span>确认发货前需上传 1 张盒面 SN 照片和 2 张打包完成照片。</span></div><div className="shipment-photo-grid">{shipmentPhotoRequirements.map((requirement) => { const photo = shipmentPhotos.find((item) => item.category === requirement.category); return <label className="shipment-photo-card" key={requirement.category}><strong>{requirement.label}</strong><small>{requirement.help}</small>{photo ? <img src={photo.dataUrl} alt={requirement.label} /> : <span>点击拍照或选择图片</span>}<input type="file" accept="image/png,image/jpeg,image/webp" capture="environment" onChange={(event) => { void selectShipmentPhoto(requirement, event.target.files?.[0]); event.currentTarget.value = ''; }} /></label>; })}</div><div className="action-list"><Button onClick={ship}>确认发货</Button></div></section>}
