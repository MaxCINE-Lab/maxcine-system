...
import { normalizeScannerValue, type OrderStatus, type SessionUser } from '@maxcine/shared';
...
type Order = { id: string; orderNo: string; dealerName?: string; storeName: string; status: OrderStatus; totalCents: number; itemCount: number; boundSerialCount?: number; pendingSerialCount?: number; itemSummary?: string; serialSummary?: string; fulfillmentCarrier?: string; fulfillmentTrackingNumber?: string; createdAt: string };
type OrderDetail = {
  order: Order & { note: string; reviewNote?: string; submittedAt: string | null; reviewedAt: string | null; salePriceCents: number | null; shippingAddress: string; customerProfile: string; screenshotDataUrl: string; packageMaterials?: string; fulfillmentCarrier?: string; fulfillmentTrackingNumber?: string; itemCount: number; boundSerialCount: number; pendingSerialCount: number };
...
const shipmentPhotoRequirements: Array<{ category: ShipmentPhotoCategory; label: string; help: string }> = [
  { category: 'box_sn', label: '条码 / SN 标签照片', help: '请拍到产品盒子表面条码或清晰 SN 标签。' },
...
function Orders({ user, route, logout, warehouse = false }: Props & { warehouse?: boolean }) {
  ...
  const tabs: Array<[string, string]> = warehouse ? [['pending_shipment', '待处理'], ['shipped', '已发货']] : [['all', '全部'], ['submitted', '待审核'], ['pending_shipment', '待发货'], ['pending_serial_binding', '待绑定 SN'], ['shipped', '已发货'], ['rejected', '已驳回']];
  return <Shell user={user} route={route} title={warehouse ? '发货' : '订单管理'} subtitle={warehouse ? '核对订单并拍摄条码 / SN 标签照片；扫码仅作为辅助，不再作为发货阻断。' : '审核订单、跟进发货，并处理已发货后的 SN 绑定。'} logout={logout}><Alert notice={notice} /><div className="filter-row">{tabs.map(([value, label]) => <button className={`filter ${status === value ? 'active' : ''}`} onClick={() => setStatus(value)} key={value}>{label}</button>)}</div><div className="table-wrap"><table><thead><tr>{warehouse ? <><th>订单编号</th><th>产品</th><th>数量</th><th>经销商</th><th>快递单号</th><th>已绑定 SN</th><th>操作</th></> : <><th>订单编号</th><th>店铺</th><th>商品</th><th>订单金额</th><th>状态</th><th>创建时间</th><th>操作</th></>}</tr></thead><tbody>{orders.map((order) => <tr key={order.id}>{warehouse ? <><td>{order.orderNo}</td><td>{order.itemSummary || '—'}</td><td>{order.itemCount}</td><td>{order.dealerName || '—'}</td><td>{order.fulfillmentTrackingNumber || '可发货后补'}</td><td>{order.serialSummary || '待管理员绑定'}</td><td><a href={`#/system/warehouse/order/${order.id}`}>{order.status === 'shipped' ? '查看' : '去发货'}</a></td></> : <><td>{order.orderNo}</td><td>{order.storeName}</td><td>{order.itemSummary || `${order.itemCount} 件`}</td><td>{money(order.totalCents)}</td><td><span className={`status status--${order.status}`}>{orderStatusText(order)}</span></td><td>{date(order.createdAt)}</td><td><a href={`#/system/admin/order/${order.id}`}>查看详情</a></td></>}</tr>)}</tbody></table>{!orders.length && <div className="empty-state"><h2>{warehouse ? '暂无待处理订单。' : '暂无符合条件的订单。'}</h2></div>}</div></Shell>;
}
...
function OrderPage({ user, route, logout, warehouse = false, orderId }: Props & { warehouse?: boolean; orderId: string }) {
  ...
  const load = useCallback(() => api<OrderDetail>(`/orders/${orderId}`).then((value) => {
    ...
    setSerialText(value.order.status === 'shipped' && value.order.pendingSerialCount > 0 ? '' : value.serials.map((item) => item.serialNumber).join('\n'));
  }).catch((error) => setNotice({ tone: 'error', text: errorText(error) })), [orderId]);
  useEffect(() => { void load(); }, [load]);
  useEffect(() => {
    if (!data || !canReview || warehouse || !(['approved', 'picking', 'packed'].includes(data.order.status) || (data.order.status === 'shipped' && data.order.pendingSerialCount > 0))) {
  ...
  const handleSerialInput = (value: string) => setSerialText(value);
  ...
  const ship = async () => {
    if (!shipmentPhotos.some((photo) => photo.category === 'box_sn')) return setNotice({ tone: 'error', text: '确认发货前请拍摄或上传条码 / SN 标签照片。' });
    if (!window.confirm(selectedSerials.length ? '确认发货吗？系统会绑定订单、SN 和保修日期。' : '确认无 SN 发货吗？订单将显示“已发货·待绑定 SN”，管理员后续根据照片补录 SN。')) return;
  ...
  const bindSerials = async () => {
    if (!selectedSerials.length) return setNotice({ tone: 'error', text: '请先选择或填写要绑定的 SN。' });
    if (!window.confirm('确认根据发货照片后补绑定这些 SN 吗？保修起算会使用原发货时间。')) return;
    await action(`/orders/${orderId}/bind-serials`, { serialNumbers: selectedSerials }, 'SN 已绑定，资产、GSX 和保修已按原发货时间初始化。');
  };
  ...
  return <Shell user={user} route={route} title={warehouse ? '发货订单' : '订单详情'} subtitle={data?.order.orderNo ?? '正在加载订单'} logout={logout}>
    ...
    {!data ? <p>正在加载…</p> : <div className="order-layout">
      <section className="panel detail-header">
        <div><span className={`status status--${data.order.status}`}>{orderStatusText(data.order)}</span><h2>{data.order.orderNo}</h2><p>{data.order.storeName}</p></div>
      ...
      {!warehouse && canReview && data.order.status === 'shipped' && data.order.pendingSerialCount > 0 && <section className="panel"><div className="panel-title"><h2>后补绑定 SN</h2><span>该订单已发货，还需绑定 {data.order.pendingSerialCount} 个 SN；保修起算将使用原发货时间 {date(data.shipment?.shippedAt)}。</span></div><p className="hint">请先查看下方条码 / SN 标签照片，再手工输入或从可用 SN 中选择。绑定成功后系统会完成 serial_numbers、资产、GSX、Internal/Public Warranty 和生命周期记录。</p><label>手工输入 SN<textarea value={serialText} onChange={(event) => handleSerialInput(event.target.value)} placeholder="每行一个 SN，可粘贴多行；也可在下方勾选可用 SN" /></label><section className="panel panel--nested"><h3>选择可用 SN</h3>{availableSerialGroups.map((group) => <div key={group.productId}><p className="hint">{group.productName} / {group.productVersion || group.sku}：需绑定 {group.quantity} 个，已选 {serialCountFor(group)} 个。</p><div className="table-wrap"><table><thead><tr><th>选择</th><th>SN</th><th>资产状态</th><th>来源 / 仓库</th><th>备注</th><th>更新时间</th></tr></thead><tbody>{group.serials.map((serial) => <tr key={serial.assetId}><td><input type="checkbox" checked={selectedSet.has(serial.serialNumber.toUpperCase())} onChange={(event) => toggleSerial(serial.serialNumber, event.target.checked)} /></td><td>{serial.serialNumber}{serial.originalSn && serial.originalSn !== serial.serialNumber && <><br /><small>原 SN：{serial.originalSn}</small></>}</td><td>{serial.assetStatus} / {serial.dataQualityStatus}</td><td>{serial.sourceChannel || '—'} / {serial.shippingWarehouse || '—'}</td><td>{serial.assetNote || serial.productNote || '—'}</td><td>{date(serial.updatedAt)}</td></tr>)}</tbody></table>{!group.serials.length && <div className="empty-state"><h2>暂无可用 SN。</h2></div>}</div></div>)}</section><div className="action-list"><Button onClick={() => void bindSerials()}>确认绑定 SN</Button></div></section>}
